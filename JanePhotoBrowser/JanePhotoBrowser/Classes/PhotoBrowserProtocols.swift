...
extension PhotoBrowserDelegate where Self: UIViewController {
    public func photoBrowser(_ photoBrowser:PhotoBrowserView, photoTappedAtIndex indexPath:IndexPath) {
        guard let dataSource = photoBrowser.dataSource, let indexPath = photoBrowser.visibleIndexPath() else { return }
        self.present(PhotoBrowserViewController.instantiate(dataSource: dataSource, delegate: photoBrowser, indexPath: indexPath), animated: true, completion: nil)
    }
